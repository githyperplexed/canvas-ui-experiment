  ...
  outColor = vec4(col, alpha);
	...
	export function createBlaze(
		...
		return {
			setOptions(next) {
				if (
					!Object.entries(next).some(([key, value]) => config[key as keyof BlazeOptions] !== value)
				)
					return;
...
<script lang="ts">
	...
	onMount(() => {
		...
		(async () => {
			...
			instance = createBlaze({ source: sourceEl, content: contentEl, output: outputEl }, options);
			if (native && !instance) {
				native = false;
				await tick();
				if (disposed) return;
