  ...
  outColor = vec4(tint * a, a);
	...
	export function createCloth(
		...
		return {
			setOptions(next) {
				if (
					!Object.entries(next).some(([key, value]) => config[key as keyof ClothOptions] !== value)
				)
					return;
...
<script lang="ts">
	...
	onMount(() => {
		...
		(async () => {
			...
			instance = createCloth({ source: sourceEl, content: contentEl, output: outputEl }, options);
			if (native && !instance) {
				native = false;
				await tick();
				if (disposed) return;
