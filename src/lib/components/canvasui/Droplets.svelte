<script module lang="ts">
	export interface DropletsOptions {
		...
		tintStrength?: number;
		/** Render the drops as a self-lit transparent overlay, ignoring captured content. Use when layering over live UI instead of wrapping content — otherwise the shader refracts an empty capture and shows nothing. */
		standalone?: boolean;
		/** URL of an image used directly as the refracted content, cover-fit to the box. Bypasses the DOM capture entirely, so raster images work everywhere. */
		image?: string;
		/** CSS color the image is colorized toward (grayscale + color blend) before upload. Empty keeps the image's own colors. */
		imageTint?: string;
	}
...
void main () {
  ...
  if (uHasContent < 0.5) {
    ...
    // Brightened relative to stock: without content refraction (the main
    // visibility driver), the stock shading is nearly invisible on dark pages.
    vec3 colF = dropCol * (0.18 + 0.65 * rim) + vec3(spec);
    float alphaF = mask * clamp(0.16 + rim * 0.6 + spec, 0.0, 1.0);
...
void main () {
  ...
  outColor = vec4(max(prev, m), 0.0, 0.0, 1.0);
	...
	export function createDroplets(
		...
		// With interaction off, the whole trail framebuffer pipeline is skipped and
		// this empty texture stands in for uTrail. Avoids multi-pass rendering on
		// drivers where framebuffer round-trips misbehave.
		const dummyTrail = gl.createTexture()!;
		gl.bindTexture(gl.TEXTURE_2D, dummyTrail);
		...
		// Optional direct image content: decoded, optionally colorized, cover-fit
		// onto an offscreen canvas, and uploaded as the refracted content texture.
		// Unlike the DOM capture, this path handles raster images everywhere.
		let backdrop: HTMLImageElement | null = null;
		let backdropSrc: string | null = null;
		let backdropDirty = false;
		let paintedTint: string | null = null;
		const backdropCanvas = document.createElement("canvas");
		const backdropCtx = backdropCanvas.getContext("2d");

		function paintBackdrop() {
			if (!backdrop || !backdropCtx) return;
			backdropCanvas.width = Math.max(output.width, 1);
			backdropCanvas.height = Math.max(output.height, 1);
			const scale = Math.max(
				backdropCanvas.width / backdrop.naturalWidth,
				backdropCanvas.height / backdrop.naturalHeight
			);
			const w = backdrop.naturalWidth * scale;
			const h = backdrop.naturalHeight * scale;
			backdropCtx.filter = config.imageTint ? "grayscale(1) contrast(1.1)" : "none";
			backdropCtx.drawImage(
				backdrop,
				(backdropCanvas.width - w) / 2,
				(backdropCanvas.height - h) / 2,
				w,
				h
			);
			backdropCtx.filter = "none";
			if (config.imageTint) {
				backdropCtx.globalCompositeOperation = "color";
				backdropCtx.fillStyle = config.imageTint;
				backdropCtx.fillRect(0, 0, backdropCanvas.width, backdropCanvas.height);
				backdropCtx.globalCompositeOperation = "source-over";
			}
			paintedTint = config.imageTint;
			backdropDirty = true;
		}

		function loadBackdrop() {
			if (config.image === backdropSrc) {
				if (backdrop && config.imageTint !== paintedTint) {
					paintBackdrop();
					wake();
				}
				return;
			}
			backdropSrc = config.image;
			backdrop = null;
			if (!config.image) return;
			const img = new Image();
			img.onload = () => {
				if (backdropSrc !== img.getAttribute("src")) return;
				backdrop = img;
				paintBackdrop();
				wake();
			};
		...
		function render(timeSec: number) {
			...
			gl!.uniform1f(uniforms.uHasContent, config.standalone ? 0 : backdrop || htmlInCanvas ? 1 : 0);
			...
			gl!.bindTexture(gl!.TEXTURE_2D, config.interactive ? trailTextures[trailIndex] : dummyTrail);
		...
		function frame(now: number) {
			...
			if (config.interactive) updateTrail(delta);
			render(elapsed);
...
<script lang="ts">
	...
	interface Props extends DropletsOptions {
		class?: string;
	...
	$effect(() => {
		// Read props eagerly: instance?. would short-circuit before tracking them
		// while the async mount is still pending, leaving this effect inert forever.
		const next = { ...options };
