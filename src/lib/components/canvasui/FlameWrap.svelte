  ...
  outColor = vec4(col, alpha);
	...
	export function createFlameWrap(
		...
		function syncCanvasSize() {
			...
			// so the flame rect stays aligned when an ancestor applies CSS 3D transforms.
			if (output.offsetWidth > 0 && box.offsetWidth > 0) {
				rect.cx = box.offsetLeft + box.offsetWidth / 2 - output.offsetLeft;
				rect.cy = output.offsetTop + output.offsetHeight - (box.offsetTop + box.offsetHeight / 2);
				rect.hx = box.offsetWidth / 2;
				rect.hy = box.offsetHeight / 2;
			}
			if (htmlInCanvas) {
