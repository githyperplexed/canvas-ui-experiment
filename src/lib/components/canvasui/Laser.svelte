<script module lang="ts">
	export interface LaserOptions {
		...
		reactivity?: number;
		/** Which end of the content the beam grows from as width changes. "center" grows symmetrically; "left" pins the left end, e.g. for progress bars. */
		anchor?: "center" | "left";
		/** Intensity profile along the beam. "fade" is the classic middle-heavy falloff; "flat" keeps constant brightness and core width along the beam with short soft caps at the ends, e.g. for progress bars. */
		envelope?: "fade" | "flat";
		/** Blank the content below the beam with the page background color (the scroll-cutoff look). Disable when using the beam decoratively. */
		cutoff?: boolean;
	}
...
void main () {
  ...
  float env = uFlat > 0.5
    ? 1.0 - smoothstep(0.92, 1.0, abs(nx))
    : pow(max(1.0 - nx * nx, 0.0), 1.5);
  ...
  vec3 col;
  float alpha;
  // The opaque bezel fill hides unrevealed content below the beam in the
  // scroll-cutoff use case; decorative uses disable it via the cutoff option.
  if (dy < 0.0 && uHasContent > 0.5 && uCutoff > 0.5) {
    col = uBezel;
    alpha = 1.0;
  } else {
    col = vec3(0.0);
	...
	export function createLaser(
		...
		function syncCanvasSize() {
			...
			if (child instanceof HTMLElement) {
				...
				// Measure in layout space (offset*), not screen space (getBoundingClientRect),
				// so the beam stays aligned when an ancestor applies CSS 3D transforms.
				const base = content.offsetLeft - output.offsetLeft + child.offsetLeft;
				const left = base + (parseFloat(style.paddingLeft) || 0);
				const right = base + child.offsetWidth - (parseFloat(style.paddingRight) || 0);
				if (right - left > 48) {
		...
		function render() {
			...
			gl!.uniform1f(uniforms.uBeamY, Math.min(Math.max(config.offset, 0) / clientH, 0.95));
			gl!.uniform1f(uniforms.uWaveAmp, Math.max(config.wave, 0) / clientH);
			const beamHalfW = Math.min(Math.max(config.width, 0.05), 1) * beamSpan * 0.5;
			gl!.uniform1f(
				uniforms.uBeamCX,
				config.anchor === "left" ? beamCX - beamSpan * 0.5 + beamHalfW : beamCX
			);
			gl!.uniform1f(uniforms.uBeamHalfW, beamHalfW);
			gl!.uniform1f(uniforms.uFlat, config.envelope === "flat" ? 1 : 0);
			gl!.uniform1f(uniforms.uCutoff, config.cutoff && config.envelope !== "flat" ? 1 : 0);
			...
			gl!.uniform1f(uniforms.uHeat, Math.max(config.heat, 0) * (0.3 + 0.7 * activity));
			gl!.uniform1f(
				uniforms.uShimmer,
		...
		return {
			setOptions(next) {
				if (
					!Object.entries(next).some(([key, value]) => config[key as keyof LaserOptions] !== value)
				)
					return;
...
<script lang="ts">
	...
	interface Props extends LaserOptions {
		class?: string;
	...
	onMount(() => {
		...
		(async () => {
			...
			instance = createLaser({ source: sourceEl, content: contentEl, output: outputEl }, options);
			if (native && !instance) {
				native = false;
				await tick();
				if (disposed) return;
