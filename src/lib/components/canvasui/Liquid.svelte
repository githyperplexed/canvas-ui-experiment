<script module lang="ts">
	export interface LiquidOptions {
		...
		rainbow?: boolean;
		/** Render the trails as a self-lit transparent overlay, ignoring captured content. Use when layering over live UI instead of wrapping content — otherwise the shader blends against an empty capture and shows nothing. */
		standalone?: boolean;
	}
...
void main () {
  ...
  if (uHasContent < 0.5) {
    ...
    // Solid trails scale the color by flow magnitude like the content path
    // does, so dense cores saturate toward white and decay back through the
    // accent instead of painting one flat tone.
    vec3 tint = uRainbow == 1.0
      ? clamp(fluid / max(mag, 1e-3), 0.0, 1.0)
      : clamp(uColor * (0.4 + mag * 1.6), 0.0, 1.0);
  ...
  outColor = vec4(velocity, 0.0, 1.0);
	...
	export function createLiquid(
		...
		function render() {
			...
			gl!.uniform1f(
				displayProgram.uniforms.uHasContent,
				config.standalone ? 0 : htmlInCanvas ? 1 : 0
			);
			blit(null);
		}
		...
		return {
			...
			setOptions(next) {
				if (
					!Object.entries(next).some(([key, value]) => config[key as keyof LiquidOptions] !== value)
				)
					return;
...
<script lang="ts">
	...
	onMount(() => {
		...
		(async () => {
			...
			instance = createLiquid({ source: sourceEl, content: contentEl, output: outputEl }, options);
			if (native && !instance) {
				native = false;
				await tick();
				if (disposed) return;
