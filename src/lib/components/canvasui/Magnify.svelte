...
<script lang="ts">
	...
	interface Props extends MagnifyOptions {
		class?: string;
	...
	let instance: MagnifyInstance | null = null;
	...
	onMount(() => {
		...
		(async () => {
			...
			instance = createMagnify({ source: sourceEl, content: contentEl, output: outputEl }, options);
			if (native && !instance) {
				native = false;
				await tick();
				if (disposed) return;
				if (!sourceEl || !contentEl || !outputEl) return;
				instance = createMagnify(
