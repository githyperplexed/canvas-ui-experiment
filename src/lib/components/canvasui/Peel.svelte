<script module lang="ts">
	...
	export interface PeelOptions {
		...
		smoothing?: number;
		/** URL of an image used as the peeling sheet, stretched to the box. Bypasses the DOM capture entirely, so it works in every browser. */
		image?: string;
	}
...
void main () {
  ...
  outColor = vec4(rgb * tex.a, tex.a);
	...
	export function createPeel(
		...
		if (under && (htmlInCanvas || config.image)) under.style.visibility = "hidden";

		// Optional direct image sheet: uploaded straight to the content texture,
		// bypassing the DOM capture (which cannot rasterize raster images).
		let sheetSrc: string | null = null;
		function loadSheet() {
			if (config.image === sheetSrc) return;
			sheetSrc = config.image;
			if (!config.image) return;
			const img = new Image();
			img.onload = () => {
				if (sheetSrc !== img.getAttribute("src")) return;
				gl!.bindTexture(gl!.TEXTURE_2D, contentTexture);
		...
		capture = () => {
			if (!htmlInCanvas || config.image) return;
			try {
				sourceCtx!.reset();
		...
		function onPointerMove(event: PointerEvent) {
			if (!htmlInCanvas && !hasTexture) return;
		...
		return {
			setOptions(next) {
				if (
					!Object.entries(next).some(([key, value]) => config[key as keyof PeelOptions] !== value)
				)
					return;
