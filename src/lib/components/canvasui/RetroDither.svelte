<script module lang="ts">
	export interface RetroDitherOptions {
		...
		followSpeed?: number;
		/** URL of an image used directly as the dither source, cover-fit to the box. Bypasses the DOM capture entirely, so it works in every browser. */
		image?: string;
	}
...
void main () {
  ...
  outColor = vec4(col, alpha);
	...
	export function createRetroDither(
		...
		// Optional direct image source: decoded and cover-fit onto an offscreen
		// canvas, then uploaded as the content texture. Unlike the DOM capture,
		// this path works in every browser and supports raster images.
		...
		function loadBackdrop() {
			if (config.image === backdropSrc) return;
		...
		function render() {
			...
			gl!.uniform3f(uniforms.uDark, config.darkColor[0], config.darkColor[1], config.darkColor[2]);
			gl!.uniform3f(
				uniforms.uLight,
				config.lightColor[0],
				config.lightColor[1],
