<script module lang="ts">
	export interface VHSOptions {
		...
		exposure?: number;
		/** URL of an image composited beneath the captured content, cover-fit to the box. Use for photos: the element capture cannot rasterize raster images, so they must enter the tape through here. */
		image?: string;
		/** CSS color the image is colorized toward (grayscale + color blend) before the tape effects. Empty keeps the image's own colors. */
		imageTint?: string;
	}
...
void main () {
  ...
  outColor = vec4(col, alpha);
	...
	export function createVHS(elements: VHSElements, options: VHSOptions = {}): VHSInstance | null {
		...
		// Direct image content: decoded, optionally colorized, cover-fit onto an
		// offscreen canvas, and uploaded straight as the content texture. Never
		// routed through the DOM capture, which drops raster images.
...
<script lang="ts">
	...
	onMount(() => {
		...
		(async () => {
			...
			instance = createVHS({ source: sourceEl, content: contentEl, output: outputEl }, options);
			if (native && !instance) {
				native = false;
				await tick();
				if (disposed) return;
